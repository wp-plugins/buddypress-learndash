    ...
    class BuddyPress_LearnDash_Loader{
        ...
        public function setup(){
            ...
            if ( class_exists( 'SFWD_LMS' ) ) {
				...
				add_action( 'learndash_update_course_access', array( $this, 'bp_learndash_user_course_start_activity' ), 100, 4 );
        ...
        public function bp_learndash_create_lesson_activity( $mid, $object_id, $meta_key, $_meta_value ) {
            ...
            $args = array(
                ...
                'action' => apply_filters( 'bp_learndash_create_lesson_activity',
                    sprintf( __( '%1$s added the lesson %2$s to the course %3$s', 'buddypress-learndash' ),
        ...
        public function bp_learndash_create_topic_activity( $mid, $object_id, $meta_key, $_meta_value ) {
            ...
            $args = array(
                ...
                'action' => apply_filters( 'bp_learndash_create_topic_activity',
                    sprintf( __( '%1$s added the topic %2$s to the lesson %3$s', 'buddypress-learndash' ),
                        $user_link, $topic_link_html, $lesson_link_html ), $user_id, $topic_id ),
				'item_id' => $group_attached,
				'component'	=> $bp->groups->id,
				'hide_sitewide'	=> true
            );
         ...
         */
        public function bp_learndash_user_course_start_activity( $user_id, $course_id, $access_list, $remove ) {
			$group_attached = get_post_meta( $course_id, 'bp_course_group', true );
			if ( empty( $group_attached ) ) {
				return;
			}
			...
			if ( !empty($bp_learndash_course_activity) && !array_key_exists( 'user_lesson_end', $bp_learndash_course_activity ) ) {
                ...
                $args = array(
                    ...
                    'action' => apply_filters( 'bp_learndash_user_lesson_end_activity',
                        sprintf( __( '%1$s completed the lesson %2$s', 'buddypress-learndash' ),
			...
			if ( !empty($bp_learndash_course_activity) && !array_key_exists( 'user_topic_end', $bp_learndash_course_activity ) ) {
                ...
                $args = array(
                    ...
                    'action' => apply_filters( 'bp_learndash_user_topic_end_activity',
                        sprintf( __( '%1$s completed the topic %2$s', 'buddypress-learndash' ),
                            $user_link, $topic_link_html ), $user_id, $lesson_id ),
                    'item_id' => $group_attached,
					'component'	=> $bp->groups->id,
					'hide_sitewide'	=> true
                );
		...
		public function bp_learndash_lesson_comment( $comment_ID, $comment_status ) {
			...
			if ( 'sfwd-lessons' == $post_type && 'approve' == $comment_status ) {
				...
				$args = array(
					'type' => 'activity_update',
					'action' => apply_filters( 'bp_learndash_user_lesson_comment_activity', sprintf( __( '%1$s commented on lesson %2$s', 'buddypress-learndash' ), $user_link, $lesson_link_html ), $comment_obj->user_id, $course_id ),
					'item_id' => $group_attached,
					'component' => $bp->groups->id,
					'hide_sitewide' => true,
		...
		public function bp_learndash_complete_quiz_activity( $quizdata, $user ) {
			...
			if ( !empty($bp_learndash_course_activity) && !array_key_exists( 'user_quiz_pass', $bp_learndash_course_activity ) ) {
				...
				$args = array(
					'type' => 'activity_update',
					'action' => apply_filters( 'bp_learndash_complete_quiz_activity', sprintf( __( '%1$s has passed the %2$s quiz with score %3$s', 'buddypress-learndash' ), $user_link, $quiz_link_html, $quiz_grade ), get_current_user_id(), $quiz_lesson_id ),
					'item_id' => $group_attached,
					'component' => $bp->groups->id,
					'hide_sitewide' => true
				);
        ...
        public function bp_learndash_add_new_setup_nav(){
            ...
            foreach($all_nav_items as $single){
                $this->bp_learndash_setup_nav($single['name'], $single['slug'], $single['screen'], $single['default_subnav']);
            }
            // create subnav item
            foreach($all_subnav_items as $single){
                $this->bp_learndash_setup_subnav($single['name'], $single['slug'], $single['parent_slug'], $single['screen']);
            }
        }

        public function bp_learndash_setup_nav($name, $slug, $screen, $default_subnav) {
        ...
        public function bp_learndash_setup_subnav($name, $slug, $parent_slug, $screen) {
